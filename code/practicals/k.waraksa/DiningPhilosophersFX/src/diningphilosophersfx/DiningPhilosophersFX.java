...
public class DiningPhilosophersFX extends Application {
    ...
    private ChopstickMonitor mon = new ChopstickMonitor();
    ...
    public void start(Stage primaryStage) {
        ...
        for (int i = 0; i < NR; i++) {
            phils[i] = new Philosopher(sticks[i], sticks[(i + NR - 1) % NR], this, mon);
    ...
    private void drawPhilosophers() {
        for (int i = 0; i < NR; i++) {
            ...
            float MaxPercentage = (float) Math.max(phils[i].getPercentageTakenLeft(), phils[i].getPercentageTakenRight());
            float MinPercentage = (float) Math.min(phils[i].getPercentageTakenLeft(), phils[i].getPercentageTakenRight());
            ...
            if (phils[i].getState() == Philosopher.state.Eating) {
                if (MinPercentage >= 0.9) {
                    ...
                    gc.strokeOval(cx - MinPercentage * PHILRADIUS, cy - MinPercentage * PHILRADIUS, 2 * MinPercentage * PHILRADIUS, 2 * MinPercentage * PHILRADIUS);
                    gc.strokeOval(cx - MaxPercentage * PHILRADIUS, cy - MaxPercentage * PHILRADIUS, 2 * MaxPercentage * PHILRADIUS, 2 * MaxPercentage * PHILRADIUS);
                }

            } else if (phils[i].getState() == Philosopher.state.Hungry) {
                gc.strokeOval(cx - 0.2f * PHILRADIUS, cy - 0.2f * PHILRADIUS, 2 * 0.2f * PHILRADIUS, 2 * 0.2f * PHILRADIUS);
    ...
    private void drawSticks() {
        ...
        for (int i = 0; i < NR; i++) {
            ...
            if (phils[i].getState() == Philosopher.state.RightStickTaken || phils[i].getState() == Philosopher.state.Eating) {
                stickRadiusDelta = 100;
                StickAngleDelta = -Math.PI / (2 * NR);
                percentage = phils[i].getPercentageTakenRight();
            } else if (phils[(i + 1) % NR].getState() == Philosopher.state.LeftStickTaken || phils[(i + 1) % NR].getState() == Philosopher.state.Eating) {
                stickRadiusDelta = 100;
                StickAngleDelta = Math.PI / (2 * NR);
